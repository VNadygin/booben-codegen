...
const visitDataValue = (boobenValue, model, file, path) => {
  ...
  if (queryField) {
    ...
    const newValuesPath = boobenValue.sourceData.queryPath.slice(1);
    const newValues = newValuesPath.reduceRight((prev, current, index) => {
      if (index === newValuesPath.length - 1) {
        return {
          [current.field]: {
            ...prev,
            ...nestedValues,
...
const walkBoobenValue = (boobenValue, model, file, path, emitFile) => {
  switch (boobenValue.source) {
    ...
    default:
      throw new Error(
        `Got BoobenValue with unknown source: ${boobenValue.source}`
      );
  }
};
