...
const generateComponentFile = (file, model, nestingLevel = 0) => {
  ...
  if (
    file.type === FileTypes.DESIGNED_COMPONENT &&
    file.queries.size === 0 &&
    file.mutations.size === 0
  ) {
    exportExpression = t.callExpression(t.identifier('withRouter'), [
      t.identifier(file.name),
    ]);
  } else if (file.queries.size > 0 || file.mutations.size > 0) {
    exportExpression = t.callExpression(t.identifier('enhance'), [
      t.identifier(file.name),
    ]);
  } else {
    exportExpression = t.identifier(file.name);
  }
