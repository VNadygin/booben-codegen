...
const { formatHandlerName, formatComponentStateSlotKey } = require('./names');
...
const generateLiteral = value => {
  ...
  if (_.isNull(value)) return t.nullLiteral(value);
...
const actionJssyValue = path => {
  if (!path) {
    throw new Error('actionJssyValue: path not specified');
  }

  const handlerName = formatHandlerName(path);

  // this./*handlerName*/
  return t.memberExpression(t.thisExpression(), t.identifier(handlerName));
};
