...
const generateVariablesAST = args => {
  const objectsAST = args.map(item => {
    // omit this from Booben ast value
    const { code } = generate(item.value);
    ...
    const astValue = template(`${value}`)().expression;
    return t.objectProperty(t.identifier(item.name), astValue);
  });
