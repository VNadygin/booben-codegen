...
const generateAttribute = (name, value, component, model) => {
  ...
  if (model.state && model.state[stateKey] && model.state[stateKey].propName === name ) {
    ...
    return {
      ...
      "value": {
        "type": "JSXExpressionContainer",
        "expression": template(`this._Component${component.id}State_${name}`)().expression
      }
    }
  }
...
const generateJSXElement = (component, model) => {
  ...
  if (model.refs && model.refs[id]) {
    ...
    attributes = [
      ...attributes,
      generateAttributeRef(model.refs[id])
    ]
